...
Процедура ЗаполнитьПланировщикНаСервере()
	ЦветУслугаОказана = Новый Цвет(255, 237, 175);
	ЦветУслугаНеОказана = Новый Цвет(255, 153, 0);
	Запрос = Новый Запрос;
	Запрос.Текст=
	"ВЫБРАТЬ
	...
	|		ПО ЗаписьКлиента.Ссылка = РеализацияТоваровИУслуг.Основание
	|ГДЕ
	...
	Пока ВыборкаСотрудники.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			ИначеЕсли Выборка.ПризнакОплаты = Перечисления.ОплатаДокумента.ЧастичноОплачен Тогда
